...
class UPFD(InMemoryDataset):
    r"""
    This is the customized version of the UPFD Dataset Class available at
    `<https://github.com/pyg-team/pytorch_geometric/blob/master/torch_geometric/datasets/upfd.py>`.
    ...
    def __init__(self, root, name, feature, news_embedding_path, split="train", transform=None,
    ...
    def get_embedding_data(self, data):
        embedding_data = []
        for key in data.keys():
            embedding_data.append(data[key].numpy())
        return torch.from_numpy(np.array(embedding_data))

    def process(self):
        x = sp.load_npz(
            osp.join(self.raw_dir, f'new_{self.feature}_feature.npz'))
        x = torch.from_numpy(x.todense()).to(torch.float)
        embedding_data = np.load(self.news_embedding_path, allow_pickle=True)
        embedding_data = self.get_embedding_data(embedding_data)
        ...
        embedding_data_slice = torch.arange(embedding_data.size(0) + 1)
        self.slices = {
            'x': node_slice,
            'edge_index': edge_slice,
            'y': graph_slice,
            'embedding_data': embedding_data_slice
        }

        edge_index -= node_slice[batch[edge_index[0]]].view(1, -1)
        self.data = Data(x=x, edge_index=edge_index, y=y, embedding_data=embedding_data)
